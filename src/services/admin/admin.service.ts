...
export class AdminService {
    ...
    async addProduct(data: ProductInput) {
        try {
            ...
            product.categories = await this.categoryRepository.findBy({id: In(data.categoryId)})
            const savedProduct = await this.productRepository.save(product)
            const productImages = await this.imageRepository.findBy({id: In(data.imageId)})
    ...
    async editProduct(id: number, data: ProductEdit) {
        try {
            ...
            if (data.categoryId) {
                existProduct.categories = await this.categoryRepository.findBy({id: In(data.categoryId)})
            }
            let dataUpdated = {
                ...existProduct,
                name: data.name,
    ...
    async deleteProduct(id: number) {
        try {
            ...
            const existInCart = await this.cartProductRepository.findOne({where: {productId: existProduct.id}})
            const existInOrder = await this.orderProductRepository.findOne({where: {productId: existProduct.id}})
